...
/// <summary>
/// The Impossible A.I is a type of A.I which are capable of destroying
/// one ship every turn. After a ship is destroyed the A.I will end the turn.
/// </summary>
public class AIMpossiblePlayer : AIPlayer
{
	private int i = 0;
	public AIMpossiblePlayer(BattleShipsGame controller) : base(controller)
	{
	}
	...
	/// <summary>
	/// SearchCoords will generate the coordinate of the player's ship location one at a time
	/// </summary>
	...
	private void SearchCoords(ref int row, ref int column)
	{
		...
		if (i == 0) { 
			row = GameController.HumanPlayer.Ship(ShipName.AircraftCarrier).Row;
			column = GameController.HumanPlayer.Ship(ShipName.AircraftCarrier).Column;
			i++;
		}
		else if (i == 1 ) { 
			row = GameController.HumanPlayer.Ship(ShipName.Battleship).Row;
			column = GameController.HumanPlayer.Ship(ShipName.Battleship).Column;
			i++;
		}
		else if (i == 2)
		{
			row = GameController.HumanPlayer.Ship(ShipName.Destroyer).Row;
			column = GameController.HumanPlayer.Ship(ShipName.Destroyer).Column;
			i++;
		}
		else if (i == 3)
		{
			row = GameController.HumanPlayer.Ship(ShipName.Submarine).Row;
			column = GameController.HumanPlayer.Ship(ShipName.Submarine).Column;
			i++;
		}
		else if (i == 4)
		{
			row = GameController.HumanPlayer.Ship(ShipName.Tug).Row;
			column = GameController.HumanPlayer.Ship(ShipName.Tug).Column;
			i++;
		}
